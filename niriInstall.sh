...
#   - alacritty, fuzzel, swaybg, JetBrainsMono Nerd Font
...
OVERWRITE_CONFIGS="${OVERWRITE_CONFIGS:-ask}"    # yes|no|ask  (replace existing niri/noctalia configs; backups are made)
...
# ------------------------------------------------------------------------------
# Phase 6: Terminal & launcher fonts (only if no config exists)
# ------------------------------------------------------------------------------
log_title "Phase 6: Alacritty & Fuzzel"

ALACRITTY_CONFIG="${HOME}/.config/alacritty/alacritty.toml"
if [[ ! -e "${ALACRITTY_CONFIG}" ]]; then
  cat > "${ALACRITTY_CONFIG}" <<ALACRITTY_EOF
# Alacritty configuration - generated by niriInstall.sh
[font]
size = 11.0
...
padding = { x = 6, y = 6 }
ALACRITTY_EOF
  log_success "Wrote ${ALACRITTY_CONFIG}"
else
  log_info "Keeping existing ${ALACRITTY_CONFIG}"
fi

FUZZEL_CONFIG="${HOME}/.config/fuzzel/fuzzel.ini"
if [[ ! -e "${FUZZEL_CONFIG}" ]]; then
  cat > "${FUZZEL_CONFIG}" <<FUZZEL_EOF
# Fuzzel configuration - generated by niriInstall.sh
[main]
...
lines=12
FUZZEL_EOF
  log_success "Wrote ${FUZZEL_CONFIG}"
else
  log_info "Keeping existing ${FUZZEL_CONFIG}"
fi
...
echo "  Mod+Return / Mod+T   Terminal            Mod+D          fuzzel"
echo "  Mod+Space            Noctalia launcher   Mod+N          Noctalia control center"
